...
function Check-DeviceCapabilities__CM_DEVCAP_SECUREDEVICE_for_WHfB_ESS 
    ...
    if ($devices) 
        ...
        ForEach ( $device in $devices )
            ...
            $deviceProperties = Get-PnpDeviceProperty -InstanceId $($device.InstanceId) -KeyName "DEVPKEY_Device_Capabilities"
            if ( $deviceProperties )
                {
...
#########################################################################
# Get EventViewer Biometrics-log - all ID=1116
# shws current configuration
#########################################################################
$WinEvents_Biometric1116_All = Get-EventViewer_WinEvents__By_LogName_and_ID -LogPath "Microsoft-Windows-Biometrics/Operational" -EventID 1116
$Today = (Get-Date).Date
$WinEvents_Biometric1116_All = $WinEvents_Biometric1116_All | Where-Object { $_.TimeCreated -ge $Today }   `

$DeleteMeFromMessage = "The Windows Biometric Service's ESS state configuration:"
# $WinEvents_Biometric1116_All = Remove-WinEvents__Message_StringToDelete -arrWinEvents $WinEvents_Biometric1116_All -StringToDelete $DeleteMeFromMessage 

# $WinEvents_Biometric1116_All | FT -Wrap | Out-String |Write-Host
$WinEvents_Biometric += $WinEvents_Biometric1116_All




#########################################################################
# Get EventViewer Biometrics-log - all ID=1600
#   The Windows Biometric Service failed to start its secure component.
#   Reason for unavailability: 12.
#   The operation failed with error: 0x80070032.                              ->  Error 0x80070032 ("The request is not supported")
#   https://learn.microsoft.com/en-us/windows/win32/secbiomet/client-error-codes
#########################################################################
$WinEvents_Biometric1600_All = Get-EventViewer_WinEvents__By_LogName_and_ID -LogPath "Microsoft-Windows-Biometrics/Operational" -EventID 1600
$Today = (Get-Date).Date
$WinEvents_Biometric1600_All = $WinEvents_Biometric1600_All | Where-Object { $_.TimeCreated -ge $Today }   `

$DeleteMeFromMessage = 'The Windows Biometric Service successfully started its secure component.'
# $WinEvents_Biometric1601_All = Remove-WinEvents__Message_StringToDelete -arrWinEvents $WinEvents_Biometric1600_All -StringToDelete $DeleteMeFromMessage 

# $WinEvents_Biometric1600_All | FT -Wrap | Out-String |Write-Host
$WinEvents_Biometric += $WinEvents_Biometric1600_All
...
$SearchText3 = "Basic"
$WinEvents_Biometric1108_WHFace_SWDRV_Basic = Query-WinEvents__Message_match_SearchString -arrWinEvents $WinEvents_Biometric1108_WHFace_SWDRV -SearchString $SearchText3
$WinEvents_Biometric1108_WHFace_SWDRV_Basic | FT -Wrap | Out-String | Write-Host
if ( $WinEvents_Biometric1108_WHFace_SWDRV_Basic )      { Write-output "ESS is working in 'Basic Mode' - please verify" }
else                                                    { Write-Output "ESS is not working in 'Basic Mode'"     }
