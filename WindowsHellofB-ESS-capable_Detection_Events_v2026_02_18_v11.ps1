...
Function Reduce-WinEvents__MessageContent_by_ID
    ...
    ForEach ( $Event in $arrWinEvents)
        ...
        Switch ( $EventID )
            ...
            1020    { 
                    # --------------   DE-DE --------------------------
                    Write-Host "1020 The Windows Biometric Service is disabled by Policy" 
                    # 
                    #  HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\System\AllowDomainPINLogon was set to 0, I've set that back to 1.
                    #  You might need to manually re-enable ESS. This can sometimes be done through Group Policy or the Registry Editor. 
                    # 
                    #  https://learn.microsoft.com/en-us/windows/client-management/mdm/passportforwork-csp
                    #  Dez 2004   For example, you can add a registry key (HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\PassportForWork) and set the value (UseEnhancedSignInSecurity) to 1

                    # 
                    # Der Windows-Biometriedienst kann eine Anforderung nicht abschließen, da  Biometrievorgänge von der aktuellen Systemrichtlinie deaktiviert wurden.
                    #
                    # nothing to shorten
                    #
                    }
            1105    { 
                    ...
                    $DeleteMeFromMessage = 'Fehler des Windows-Biometriediensts beim Initialisieren einer Adapterbinärdatei: '
                   # $arrWinEvents = Remove-WinEvents__Message_StringToDelete -arrWinEvents $arrWinEvents -StringToDelete $DeleteMeFromMessage 
                    
                    $DeleteMeFromMessage = 'des Windows-Biometriediensts beim '
                    $arrWinEvents = Remove-WinEvents__Message_StringToDelete -arrWinEvents $arrWinEvents -StringToDelete $DeleteMeFromMessage 
                    
                    $DeleteMeFromMessage = ' des Windows-Biometriediensts beim '
                    $arrWinEvents = Remove-WinEvents__Message_StringToDelete -arrWinEvents $arrWinEvents -StringToDelete $DeleteMeFromMessage 
                    
                     $DeleteMeFromMessage = 'einer Adapterbinärdatei:'
                     $arrWinEvents = Remove-WinEvents__Message_StringToDelete -arrWinEvents $arrWinEvents -StringToDelete $DeleteMeFromMessage 
                   

                    $DeleteMeFromMessage = 'Fehler der "Sensor Adapter"-Initialisierungsdes Moduls. '
                    $arrWinEvents = Remove-WinEvents__Message_StringToDelete -arrWinEvents $arrWinEvents -StringToDelete $DeleteMeFromMessage 
                    
                    $DeleteMeFromMessage = 'Im Detailbereich finden Sie Informationen zur fehlerhaften Konfiguration.'
                    $arrWinEvents = Remove-WinEvents__Message_StringToDelete -arrWinEvents $arrWinEvents -StringToDelete $DeleteMeFromMessage 
            ...
            1108    { 
                    ...
                    Write-Host "1108 ... created a Biometric Unit for sensor ... / ... The sensor's mode is ...."   
            ...
            1109    { 
                    ...
                    $DeleteMeFromMessage = 'einer Biometrie-Einheit für Sensor '
                    $arrWinEvents = Remove-WinEvents__Message_StringToDelete -arrWinEvents $arrWinEvents -StringToDelete $DeleteMeFromMessage 
            ...
            1600    { 
                    Write-Host "1600 Fehler beim Starten der sicheren Komponente des Windows-Biometriediensts. Grund für Nichtverfügbarkeit: 76.Vorgangsfehler: 0x80070032."                 
                    #
                    $DeleteMeFromMessage = 'Fehler beim Starten der sicheren Komponente des Windows-Biometriediensts.'
                    $arrWinEvents = Remove-WinEvents__Message_StringToDelete -arrWinEvents $arrWinEvents -StringToDelete $DeleteMeFromMessage 
...
# on DELL Lat  5410 =>   Id = '1105','1108','1109',       '1600'
...
ForEach ( $ID in $AllBioMetricEventIDs)
   ...
   # $WinEvents_Biometric_ID  | select ID, Message | FT -wrap
    ...
    #  ID 1108
        ...
        #  ID 1108
        $SearchText =     "The sensor's mode is ""Advanced,"" its pool-type is ""System,"" and it's isolated in a ""Local System"" process."
        $NewText    = "`r`nThe sensor's mode is ""Advanced,"" its pool-type is ""System,"" and it's isolated in a ""Local System"" process."
        ForEach ( $Event in $WinEvents_Biometric_ID)
            {
            if ( $Event.Message -like "*$SearchText*"  )
                {
    ...
    # --------------   DE-DE --------------------------
        ...
        ForEach ( $Event in $WinEvents_Biometric_ID)
         ...
         #  ID 1108
        $SearchText =     "Der Sensor befindet sich im Modus ""Advanced"". Der Pooltyp des Sensors lautet ""System"", und er ist in einem Local System-Prozess isoliert."
        $NewText    = "`r`nDer Sensor befindet sich im Modus ""Advanced"". Der Pooltyp des Sensors lautet ""System"", und er ist in einem Local System-Prozess isoliert."
        ForEach ( $Event in $WinEvents_Biometric_ID)
            {
            if ( $Event.Message -like "*$SearchText*"  )
                {
        ...
        #  ID 1108
        $SearchText =     "Der Sensor befindet sich im Modus ""Basic"". Der Pooltyp des Sensors lautet ""System"", und er ist in einem Local System-Prozess isoliert."
        $NewText    = "`r`nDer Sensor befindet sich im Modus ""Basic"". Der Pooltyp des Sensors lautet ""System"", und er ist in einem Local System-Prozess isoliert."
        ForEach ( $Event in $WinEvents_Biometric_ID)
            {
            if ( $Event.Message -like "*$SearchText*"  )
                {
...
# -------------------------------------------------------
$SearchText = "The sensor's mode is ""Basic,"" its pool-type is ""System,"" and it's isolated in a ""Local System"" process."
$WinEvents_Biometric_Sensor_Basic_EN_US = Query-WinEvents__Message_match_SearchString -arrWinEvents $WinEvents_Biometric_All -SearchString $SearchText

$SearchText = "Der Sensor befindet sich im Modus ""Basic"". Der Pooltyp des Sensors lautet ""System"", und er ist in einem Local System-Prozess isoliert."
$WinEvents_Biometric_Sensor_Basic_DE_DE = Query-WinEvents__Message_match_SearchString -arrWinEvents $WinEvents_Biometric_All -SearchString $SearchText

$WinEvents_Biometric_Sensor_Basic = $WinEvents_Biometric_Sensor_Basic_EN_US
$WinEvents_Biometric_Sensor_Basic += $WinEvents_Biometric_Sensor_Basic_DE_DE

Write-Host "==========================================================================="
Write-Host "============ filtered by 'BASIC' + 'System' +  'Local System'  ============"
if ( $WinEvents_Biometric_Sensor_Basic )
    ...
    {
    Write-Host "`r`n         no events detected `r`n`r`n"
    }
...
Write-Host "==========================================================================="
Write-Host "========== filtered by 'Advanced' + 'System' +  'Local System'  ==========="
Write-Host "... indicates that Windows ESS is currently not fully active or it is operating in a fallback state."
Write-Host "... ensure BIOS settings have virtualization and security features (like HSTI) enabled"

$SearchText = "Der Sensor befindet sich im Modus ""Advanced"". Der Pooltyp des Sensors lautet ""System"", und er ist in einem Local System-Prozess isoliert."
$WinEvents_Biometric_Sensor_Advanced_DE_DE = Query-WinEvents__Message_match_SearchString -arrWinEvents $WinEvents_Biometric_All -SearchString $SearchText

$SearchText = "The sensor's mode is ""Advanced,"" its pool-type is ""System,"" and it's isolated in a ""Local System"" process."
$WinEvents_Biometric_Sensor_Advanced_EN_US = Query-WinEvents__Message_match_SearchString -arrWinEvents $WinEvents_Biometric_All -SearchString $SearchText


$WinEvents_Biometric_Sensor_Advanced  = $WinEvents_Biometric_Sensor_Advanced_EN_US
$WinEvents_Biometric_Sensor_Advanced += $WinEvents_Biometric_Sensor_Advanced_DE_DE
...
Write-Host "==========================================================================="
Write-Host "========== filtered by 'Advanced' + 'System' +  'Virtual Secure Mode' ==== "
Write-Host """Virtual Secure Mode"" for maximum protection."

$SearchText = "The sensor's mode is ""Advanced,"" its pool-type is ""System,"" and it's isolated in a ""Virtual Secure Mode"" process."
$WinEvents_Biometric_Sensor_Advanced_EN_US = Query-WinEvents__Message_match_SearchString -arrWinEvents $WinEvents_Biometric_All -SearchString $SearchText

$WinEvents_Biometric_Sensor_Advanced_DE_DE = $Null
...
if ( $WinEvents_Biometric_Sensor_Advanced )
    ...
    {
    Write-Host "`r`n         no events detected   - Script requires imporvment - DE-DE text is unknown `r`n`r`n"
    }
...
if ( $WinEvents_Biometric_InstanceID -eq $Null )
    {
...
$SearchText = "Facial Recognition (Windows Hello) Software Device"
$WinEvents_Biometric_DeviceName = Query-WinEvents__Message_match_SearchString -arrWinEvents $WinEvents_Biometric_All -SearchString $SearchText
if     ( $WinEvents_Biometric_DeviceName )
    {
    Write-Host "==========================================================================="
    Write-Host "================ filtered by DeviceName  from today ======================="
    Write-Host $SearchText
