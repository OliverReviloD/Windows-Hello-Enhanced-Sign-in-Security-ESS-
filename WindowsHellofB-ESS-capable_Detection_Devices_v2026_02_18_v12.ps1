...
# Get-PnpDevice   -Status OK -ErrorAction SilentlyContinue  | select FriendlyName,Class | Sort FriendlyNam

$fingerprintDevices = Get-PnpDevice -Class Biometric,CVAULT -Status OK -ErrorAction SilentlyContinue `
...
if ($fpNames) 
    ...
    foreach ($FingerprintDevice in $fingerprintDevices) { 
        ...
        # #########################################################################################
        #
        # Class = CVAULT    =>       Service  =  cvusbdrv
        #
        if ( $DeviceObject.Class -eq  'CVAULT' )
            {
            $RegPath = "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Enum\" + $($DeviceObject.InstanceId) + "\Device Parameters"   # \WinBio\Configurations
            if ( $MyDebug ) { Write-Host "`r`n`r`nRegKey '$RegPath' has to exist" }
            ...
            if ( $RegPathExists )
                ...
                if ( $RegPathExists )
                    ...
                    {
                    if ( $MyDebug ) { Write-Host "RegPath is missing " -ForegroundColor yellow }
                    }
                }
            else
                {
...
$hostControllers = @()
$hostControllers = Get-PnpDevice  -ErrorAction SilentlyContinue | Where-Object { $_.FriendlyName -like "*eXtensible Host Controller*"  -or $_.FriendlyName -like "*eXtensible-Hostcontroller*"   } 

# trying to reduce language pack impact
$hostControllers = Get-PnpDevice  -ErrorAction SilentlyContinue | Where-Object { ( $_.FriendlyName -like "*eXtensible*" ) -and $_.Service -eq 'USBXHCI'   } 

if ( -not $hostControllers )
    {

    # Get-PnpDevice  -ErrorAction SilentlyContinue  | select FriendlyName | sort FriendlyName
    #
    # DE-DE   Intel(R) USB 3.1 eXtensible-Hostcontroller – 1.10 (Microsoft) 
    # EN-US   Intel(R) USB 3.1 eXtensible Host Controller – 1.10 (Microsoft) 
    #

    Write-Warning "Pnp-Device-Query for    ""*eXtensible Host Controller*""    or    ""*eXtensible-Hostcontroller*""   returned NULL  "
    Write-Warning " - check your device manager for correct name language - this is lang pack related "
    Throw
    }
...
$AllDeviceObjects  | select deviceName,InstanceId,Class,Service,Capabilities_SECUREDEVICE,Capabilities_FPR_Reg_Summary | ft | out-string | write-Host
